...
def synthetic(n, a):
    X1r = np.random.randn(n)
    X2r = np.random.randn(n)
    Xpr = np.random.randn(n)
    Xp = X1r + X2r + 0.1*np.random.randn(n)
    y = pd.Series(((a*Xp + (1-a)*Xpr + 0.1*np.random.randn(n)) > 0)*1)
    X = pd.DataFrame({
        'X1r': X1r,
        'X2r': X2r,
        'Xpr': Xpr,
...
for a in [0, 0.25, 0.5, 0.75, 1]:
    ...
    for k in range(n_iter):
        ...
        X_train, y_train, pi_features = synthetic(n_tr, a)
        X_test, y_test, pi_features = synthetic(n_test, a)
...
dff.to_csv('synth' + '_'  + lc + '_' + str(n_tr) + '_' + str(n_test) + '_' + str(epo) + '_' + str(bs)  + '_' +  str(pat)  + '_' + str(n_iter)+ '.csv')
