...
def pen_based():
    ...
    df_train = pd.read_csv('data/UCI/pen/pendigits.tra', names = col)
    df_test = pd.read_csv('data/UCI/pen/pendigits.tes', names = col)  
...
def statlog_shuttle():
    ...
    df_train = pd.read_csv('data/UCI/statlog+shuttle/shuttle.trn', names = col, sep = ' ')
    df_test = pd.read_csv('data/UCI/statlog+shuttle/shuttle.tst', names = col, sep = ' ')
    df = pd.concat([df_train, df_test]).reset_index(drop = True)
    X = df.drop('out', axis = 1)
    y = df['out']
    y = pd.Series((y == 1)*1)
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def pollen():
    data, meta = arff.loadarff('data/fs_paper/pollen.arff')
    df = pd.DataFrame(data)
    patron = r"'(.*?)'"
    df['binaryClass'] = np.ravel([re.findall(patron, str(i)) for i in list(df['binaryClass'])])
    X = df.drop('binaryClass', axis = 1)
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def wilt():
    data, meta = arff.loadarff('data/fs_paper/wilt.arff')
    df = pd.DataFrame(data)
    patron = r"'(.*?)'"
    df['Class'] = np.ravel([re.findall(patron, str(i)) for i in list(df['Class'])])
    X = df.drop('Class', axis = 1)
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def ada_agnostic():
    data, meta = arff.loadarff('data/fs_paper/ada_agnostic.arff')
    df = pd.DataFrame(data)
    patron = r"'(.*?)'"
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def kc1():
    data, meta = arff.loadarff('data/fs_paper/kc1.arff')
    df = pd.DataFrame(data)
    patron = r"'(.*?)'"
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def elect():
    data, meta = arff.loadarff('data/fs_paper/electricity-normalized.arff')
    df = pd.DataFrame(data)
...
#url: https://www.openml.org/search?type=data&status=active&id=987
def phishing_big():
    data, meta = arff.loadarff('data/fs_paper/phishing_big.arff')
    df = pd.DataFrame(data)
    patron = r"'(.*?)'"
    for j in df.columns:
